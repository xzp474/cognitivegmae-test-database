...
class raiseddata extends StatefulWidget{
  @override
  State<StatefulWidget> createState()=>raiseddataState();
}

class raiseddataState extends State<raiseddata>{
  ...
  Widget build(BuildContext context) {
    ...
    var userpassword = '';
    if(allData.userData.userID.tested.coged.raised=='0'){
      allData.userData.userID.tested.coged.raised='1';}
    allData.userData.userID.times.cogTimes.raised =
        (int.parse(allData.userData.userID.times.cogTimes.raised)+1)
            .toString();
    allData.userData.userID.score.cogScore.raised.add(
            (int.parse(allData.userData.userID.questionData.raised.p1_1.last)
            +int.parse(allData.userData.userID.questionData.raised.p1_2.last)
            +int.parse(allData.userData.userID.questionData.raised.p1_3.last)
    ).toString());

    allData.lineRaised.add(
        FlSpot(double.parse(allData.userData.userID.times.cogTimes.raised),
            double.parse(allData.userData.userID.score.cogScore.raised.last)));
    allData.userData.userID.testTime.cogTime.raised.add('${DateTime.now()}');
    ...
    Future<String> readBase() async {
      await readData.child('myapp/user/${allData.userID}/測驗次數/認知測驗/反應力').onValue.listen((event) {
        allData.userData.userID.times.cogTimes.raised = event.snapshot.value.toString();
      });
